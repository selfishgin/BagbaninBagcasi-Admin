...
import SeeAllCustomer from "../customer/SeeAllCustomer";
...
export default function BasicTables({tableData}) {
    ...
    return (
        <div className="space-y-6">
                  ...
                  <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
                    {tableData.map((item, index) => (
                      <TableRow key={item.id}>
                        ...
                        <TableCell className="px-4 py-3 text-start">
                          <button onClick={() => handleSeeAll(item.customer)} 
          ...
          <SeeAllCustomer isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} customer={selectedCustomer} />
        </div>
    );
}
